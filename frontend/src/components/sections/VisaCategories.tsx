        ...
        >
          {visaCategories.map((category, i) => {
            ...
            return (
              <Link
                key={category.id}
                href={`/visa/${category.id}`}
                className="group relative block overflow-hidden rounded-2xl border border-border/60 bg-surface p-6 shadow-md transition-all duration-300 hover:border-coral/30 hover:shadow-2xl hover:shadow-coral/30"
              >
                <motion.div
                  initial={{ opacity: 0, y: 30 }}
                  whileInView={{ opacity: 1, y: 0 }}
                ...
                >
                  ...
                  <span className="mt-4 inline-flex items-center gap-1 text-xs font-semibold text-coral transition-colors group-hover:text-coral-light">
                    Learn More
